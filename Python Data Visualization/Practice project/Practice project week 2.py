...
def get_county_attributes(svg_file_name):
    ...
    county = [(county_id, county_d) for county_id, county_d in zip(path_id, path_d) ]
    doc.unlink()
    return county
...
def get_boundary_coordinates(boundary_data):
    ...
    Ignores 'M', 'L, 'z'
    """
    data = boundary_data.replace("M", "")
    data = data.replace("L", "")
    data = data.replace("z", "")
    coord = data.split()
    coord_list = [(float(ele.split(",")[0]), float(ele.split(",")[1])) for ele in coord]
    return coord_list
...
def process_county_attributes(svg_file_name, csv_file_name):
    ...
    county_list = get_county_attributes(svg_file_name)
    name_list = [county[0] for county in county_list]
    buondary_coord_list = [get_boundary_coordinates(county[1]) for county in county_list]
    center_list = [compute_county_center(coord) for coord in buondary_coord_list]
    county_center_list = [(name, center[0], center[1]) for name, center in zip(name_list, center_list)]
    with open(csv_file_name, "w", newline="") as csvfile:
        data = csv.writer(csvfile, delimiter= ",", quoting=csv.QUOTE_MINIMAL )
        for row in county_center_list:
            data.writerow(row)
    # Ouput CSV file
...
def test_same_file(file1, file2):
...
test_same_file("USA_Counties_with_FIPS_and_centers.csv", "USA_Counties_with_FIPS_and_centers_solution.csv")
